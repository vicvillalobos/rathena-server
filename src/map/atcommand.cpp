...
ACMD_FUNC(mapmove)
{
	...
	if (m < 0)
	{											 // m < 0 means on different server! [Kevin]
...
ACMD_FUNC(who)
{
	...
	{
		if (!((pc_has_permission(pl_sd, PC_PERM_HIDE_SESSION) || pc_isinvisible(pl_sd)) && pc_get_group_level(pl_sd) > level))
...
/*==========================================
 * @costumeitem command (usage: @item <itemdid> <quantity>) (Por Thomi (male sal))
 *------------------------------------------*/
...
ACMD_FUNC(costumeitem)
{
	...
	while (itemlist != nullptr)
	{
		...
		// Solo permitir headgears como costume
		if (!(item->equip & (EQP_HEAD_LOW | EQP_HEAD_MID | EQP_HEAD_TOP |
							 EQP_COSTUME_HEAD_LOW | EQP_COSTUME_HEAD_MID | EQP_COSTUME_HEAD_TOP)))
		{
			clif_displaymessage(fd, "Only headgear items can be given as costume.");
			return -1;
		}

		items.push_back(item);
		itemlist = strtok(nullptr, ":");
	}
...
ACMD_FUNC(param)
{
	...
	if (stat == ARRAYLENGTH(parameter_names))
	{												// normally impossible...
		clif_displaymessage(fd, msg_txt(sd, 1013)); // Please enter a valid value (usage: @str/@agi/@vit/@int/@dex/@luk <+/-adjustment>).
		return -1;
	}
...
ACMD_FUNC(jailfor)
{
	...
	// Added by Coltaro
	if (pl_sd->sc.getSCE(SC_JAILED) && pl_sd->sc.getSCE(SC_JAILED)->val1 != INT_MAX)
...
ACMD_FUNC(jailtime)
{
	...
	if (sd->sc.getSCE(SC_JAILED)->val1 <= 0)
...
ACMD_FUNC(showmobs)
{
	...
	if (status_has_mode(&mob->status, MD_STATUSIMMUNE) && !pc_has_permission(sd, PC_PERM_SHOW_BOSS))
...
ACMD_FUNC(langtype)
{
	...
	if (sscanf(message, "%3s", langstr) >= 1)
	{
		...
		if (msg_checklangtype(lang, false) == 1)
		{ // Verify it's enabled and set it
			char output[100];
		...
		{											   // defined langage but failed check
...
#include <custom/atcommand_def.inc>
	...
	{
		if (atcommand_exists(atcommand_base[i].command))
